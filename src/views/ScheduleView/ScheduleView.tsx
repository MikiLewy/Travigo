...
import { create, edit, info, close } from 'features/modal/modalSlice';
import { useDispatch } from 'react-redux';

const ScheduleView: React.FC = () => {
  const { id } = useParams();
  const { token } = useSelector((state: State) => state.auth);
  const { isEditing, isOpen, title, isInfo } = useSelector((state: State) => state.modal);
      ...
      <Wrapper>
        ...
        <ContentWrapper>
          <TitleWrapper>
            ...
            <Button onClick={() => dispatch(create())}>Add new</Button>
          </TitleWrapper>
          <ScheduleList>
            {events.length > 0 ? (
              events.map(({ _id, title }) => (
                <ScheduleListItem key={_id}>
                  <h4>{title}</h4>
                  <ButtonsWrapper>
                    <Link to={`/schedule/${_id}`} onClick={() => dispatch(info())}>
                    ...
                    <Link to={`/schedule/${_id}`} onClick={() => dispatch(edit())}>
      ...
      <Modal isOpen={isOpen}>
        {isInfo ? (
          <>
            ...
            <Button isWhite onClick={() => dispatch(close())}>
              Close
            </Button>
          </>
        ) : (
            ...
            <Content>
              <Form onSubmit={handleSubmit(handleAddEvent)}>
                ...
                <FormField
                  ...
                  id="image"
                  type="file"
                  {...register('image', { required: isEditing ? false : true })}
